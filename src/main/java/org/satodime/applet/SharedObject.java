...
public class SharedObject {
    ...
    static short ndefStaticDataFileSize = 0x2C;
    static byte[] ndefStaticDataFile = {
            //(byte) 0x2c,
    ...
    byte ndef_policy = (byte) 2; // default to dynamic NDEF

    /** NDEF file for dynamic url**/
    short ndefDynamicDataFileSize;
    byte[] ndefDynamicDataFile;
    ...
    void init(byte[] buffer, byte nb_slots, boolean is_coa) {
        ...
        // Allocate the memory
        this.ndefDynamicDataFileSize = (short)(offset_authentikey_sig + SIZE_SIGNATURE);
        this.ndefDynamicDataFile = new byte[this.ndefDynamicDataFileSize];

        // populate ndef record with the fixed parts
        // header
        short offset = 0;
        Util.setShort(ndefDynamicDataFile, offset, (short)(this.ndefDynamicDataFileSize -2));
        offset += (short) 2;
        ndefDynamicDataFile[offset++] = (byte) 0xC1; // Header for Long URL
        ndefDynamicDataFile[offset++] = (byte) 0x01; // Type Length
        ndefDynamicDataFile[offset++] = (byte)0x00; // Short Payload Length (4b)
        ndefDynamicDataFile[offset++] = (byte)0x00; // Short Payload Length (4b)
        Util.setShort(ndefDynamicDataFile, offset, (short)(this.ndefDynamicDataFileSize -9));
        offset += (short) 2;
        ndefDynamicDataFile[offset++] = (byte) 0x55; // Record Type 'U'
        ndefDynamicDataFile[offset++] = (byte) 0x04;// URI Identifier Code (https://)
        // base URL
        Util.arrayCopy(BASE_URL, (short) (0), ndefDynamicDataFile, offset, (short) BASE_URL.length);
        offset += BASE_URL.length;
        // skip nonce
        offset += SIZE_NONCE;
        // cardtype
        if (is_coa){
            ndefDynamicDataFile[offset++] = (byte) 'A';
        } else {
        ...
        ndefDynamicDataFile[offset++] = HEX[Satodime.PROTOCOL_MAJOR_VERSION & 0x0F];
        ndefDynamicDataFile[offset++] = HEX[(short)((Satodime.PROTOCOL_MINOR_VERSION>>4) & 0x0F)];
        ndefDynamicDataFile[offset++] = HEX[Satodime.PROTOCOL_MINOR_VERSION & 0x0F];
        ndefDynamicDataFile[offset++] = HEX[(short)((Satodime.APPLET_MAJOR_VERSION>>4) & 0x0F)];
        ndefDynamicDataFile[offset++] = HEX[Satodime.APPLET_MAJOR_VERSION & 0x0F];
        ndefDynamicDataFile[offset++] = HEX[(short)((Satodime.APPLET_MINOR_VERSION>>4) & 0x0F)];
        ndefDynamicDataFile[offset++] = HEX[Satodime.APPLET_MINOR_VERSION & 0x0F];

        // NDEF authentikey (dummy value until personalization)
        Util.arrayFillNonAtomic(ndefDynamicDataFile, offset, SIZE_PUBKEY, (byte)'0');
        offset += SIZE_PUBKEY;

        // nb_slots
        ndefDynamicDataFile[offset++] = HEX[this.nb_slots & 0x0F];// nb_slots should be <= 0x0F

        // save default slot info (to be populated)
        Util.arrayFillNonAtomic(ndefDynamicDataFile, this.offset_first_slot, (short)(this.nb_slots*SIZE_SLOT), (byte)'0');
    }
    ...
    void populateNdefDataFile() {
        ...
        for (short i=0; i<8; i++){
            ...
            tmpByte = tmpBuffer[i];
            ndefDynamicDataFile[offset++] = HEX[(tmpByte >> 4) & 0x0F];
            ndefDynamicDataFile[offset++] = HEX[tmpByte & 0x0F];
        }
        ...
        short sign_size= sigECDSA.sign(ndefDynamicDataFile, this.offset_nonce, this.size_message_to_sign, tmpBuffer, (short)0);
        ...
        sign_size -=(short)70;
        ndefDynamicDataFile[this.offset_authentikey_sig_size] = HEX[sign_size & 0x0F];// should be 0-2
    ...
    void setSlotState(byte slot_index, byte slot_state) {
        ...
        ndefDynamicDataFile[offset] = HEX[slot_state & 0x0F]; // should be 0, 1 or 2
    }
    ...
    void resetSlot(byte slot_index){
        short offset = (short)(this.offset_first_slot + slot_index * SIZE_SLOT);
        Util.arrayFillNonAtomic(ndefDynamicDataFile, offset, SIZE_SLOT, (byte)'0');
    }
